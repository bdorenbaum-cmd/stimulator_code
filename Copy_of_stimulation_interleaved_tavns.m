...
% CHANGE FILE NAME TO MATCH PATIENT
...
for i = 1:size(order, 1)
    ...
    if stim.intensity == -1
        pWaveForm = struct('ChannelNumber',stim.anode,'Action',1,'PathToFile','C:\Users\MxN-33\Desktop\SMI_HDSC_SW_V3010\waveforms_files\square_sham_1.6.txt'); 

    elseif stim.intensity == -2
    ...
    while track == 0
        try
            trigger = load("\\HodicsHP\LMRI_V6_taVNS\triggers\trigger_var.mat");
        catch err
            disp(err.message) %see what is failing
            continue
        end
        ...
        elseif trigger.trigger_var == 1 & track == 0
            ...
            track = 1;
            trigger_var = 0;
            save("\\HodicsHP\LMRI_V6_taVNS\triggers\trigger_var.mat", "trigger_var")

            if i == 1
                t = tic;
            end

            X = ['Started Stimulation Number ', num2str(i),' At time ', num2str(toc(t)), ' seconds'];
            disp(X)
            pause(5)
        end
    end
    ...
    clearvars -except info lib outlet opts order t i
end
